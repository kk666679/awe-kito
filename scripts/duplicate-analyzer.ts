...
class DuplicateAnalyzer {
  private fileHashes: Map<string, FileInfo[]> = new Map();
  private folderHashes: Map<string, FolderInfo[]> = new Map();
  private processedFiles: Set<string> = new Set();
  private processedFolders: Set<string> = new Set();

  constructor(private rootPath: string, private excludePatterns: string[] = []) {}

  async analyze(): Promise<DuplicateGroup[]> {
  ...
  private async processFolder(folderPath: string): Promise<void> {
    if (this.processedFolders.has(folderPath)) return;
  ...
  private async computeFolderHash(folderInfo: FolderInfo): Promise<string> {
    const hash = crypto.createHash('sha256');
  ...
  private findDuplicates(): DuplicateGroup[] {
...
async function main() {
  ...
  const reportPath = path.join(rootPath, 'duplicate-analysis-report.md');
