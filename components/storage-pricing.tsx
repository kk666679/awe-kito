import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Check, Cloud, Zap, Shield } from "lucide-react"

const pricingTiers = [
  {
    name: "Starter",
    description: "Perfect for small businesses",
    icon: Cloud,
    price: "RM 29",
    period: "/month",
    features: ["Up to 100GB storage", "2 cloud providers", "Basic analytics", "Email support", "Standard encryption"],
    cta: "Start Free Trial",
    popular: false,
  },
  {
    name: "Professional",
    description: "For growing businesses",
    icon: Zap,
    price: "RM 99",
    period: "/month",
    features: [
      "Up to 1TB storage",
      "All cloud providers",
      "Advanced analytics",
      "Priority support",
      "Advanced encryption",
      "Cost optimization",
      "Automated workflows",
    ],
    cta: "Start Free Trial",
    popular: true,
  },
  {
    name: "Enterprise",
    description: "For large organizations",
    icon: Shield,
    price: "Custom",
    period: "pricing",
    features: [
      "Unlimited storage",
      "All cloud providers",
      "Custom analytics",
      "24/7 dedicated support",
      "Enterprise encryption",
      "Advanced governance",
      "Custom integrations",
      "SLA guarantees",
    ],
    cta: "Contact Sales",
    popular: false,
  },
]

export function StoragePricing() {
  return (
    <section className="container mx-auto px-4 py-16 sm:py-20">
      <div className="mb-12 text-center">
        <h2 className="text-3xl font-bold text-white sm:text-4xl">Storage Management Pricing</h2>
        <p className="mt-4 text-lg text-neutral-300">Choose the right plan for your multi-cloud storage needs</p>
      </div>

      <div className="grid gap-8 md:grid-cols-3">
        {pricingTiers.map((tier, index) => {
          const IconComponent = tier.icon
          return (
            <Card
              key={index}
              className={`liquid-glass border backdrop-blur-xl relative ${
                tier.popular ? "border-lime-300/50 bg-lime-300/10 scale-105" : "border-white/10 bg-white/5"
              }`}
            >
              {tier.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  <span className="bg-lime-300 text-black px-3 py-1 rounded-full text-xs font-semibold">
                    Most Popular
                  </span>
                </div>
              )}

              <CardHeader className="text-center">
                <div className="mx-auto mb-4 rounded-lg bg-white/10 p-3 w-fit">
                  <IconComponent className={`h-8 w-8 ${tier.popular ? "text-lime-300" : "text-white"}`} />
                </div>
                <CardTitle className="text-xl text-white">{tier.name}</CardTitle>
                <p className="text-sm text-neutral-300">{tier.description}</p>
                <div className="mt-4">
                  <div className={`text-3xl font-bold ${tier.popular ? "text-lime-300" : "text-white"}`}>
                    {tier.price}
                    <span className="text-lg font-normal text-neutral-400">{tier.period}</span>
                  </div>
                </div>
              </CardHeader>

              <CardContent>
                <ul className="space-y-3 mb-6">
                  {tier.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-center gap-3">
                      <Check className={`h-4 w-4 ${tier.popular ? "text-lime-300" : "text-white"}`} />
                      <span className="text-sm text-neutral-300">{feature}</span>
                    </li>
                  ))}
                </ul>

                <Button
                  className={`w-full rounded-full ${
                    tier.popular
                      ? "bg-lime-400 text-black hover:bg-lime-300"
                      : "bg-white/10 text-white hover:bg-white/20"
                  }`}
                >
                  {tier.cta}
                </Button>
              </CardContent>
            </Card>
          )
        })}
      </div>

      {/* Additional Info */}
      <div className="mt-16 text-center">
        <Card className="liquid-glass border border-white/10 bg-white/5 backdrop-blur-xl">
          <CardContent className="p-8">
            <h3 className="mb-4 text-xl font-bold text-white">All Plans Include</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-neutral-300">
              <div className="flex items-center gap-2">
                <Check className="h-4 w-4 text-lime-300" />
                99.9% uptime SLA
              </div>
              <div className="flex items-center gap-2">
                <Check className="h-4 w-4 text-lime-300" />
                Data encryption
              </div>
              <div className="flex items-center gap-2">
                <Check className="h-4 w-4 text-lime-300" />
                API access
              </div>
              <div className="flex items-center gap-2">
                <Check className="h-4 w-4 text-lime-300" />
                Mobile apps
              </div>
            </div>
          </CardContent>
        </Card>
      </div>
    </section>
  )
}
