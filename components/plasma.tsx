...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  useEffect(() => {
    ...
    const loadOGL = async () => {
      try {
        ...
        // --- Resize handling with error suppression ---
        const setSize = () => {
          try {
            const rect = containerRef.current!.getBoundingClientRect()
        ...
        const debouncedSetSize = () => {
          ...
          resizeTimeout = setTimeout(setSize, 16) // Debounce to next frame
        }
  ...
  if (hasError) {
    return (
      <div className="plasma-container pointer-events-none will-change-transform flex items-center justify-center">
        <div className="text-muted-foreground text-sm">Unable to load plasma effect</div>
      </div>
    )
  }
